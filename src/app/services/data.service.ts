...
export class DataService {
  ...
  async refreshRainData() {
    const data = await this.getRainPerRegion(this.start, this.end).toPromise();
    if (data) {
      this.initRainPerRegion = this.mapperService.weatherToRainPerRegion(data);
    }
  }
  ...
  getWindPerStation() {
    const query =
      this.queryBuilderService.buildQuery_getAllStationsAvgWindSpeed();
    const url = `${this.apiUrl}?query=${encodeURIComponent(query)}`;
    return this.http.get<IWeather>(url);
  }
  getWindDirectionPerStation() {
    const query =
      this.queryBuilderService.buildQuery_getAllStationsAvgWindDirection();
    const url = `${this.apiUrl}?query=${encodeURIComponent(query)}`;
    return this.http.get<IWeather>(url);
  }
  ...
  getHumidityPerStation() {
    const query =
      this.queryBuilderService.buildQuery_getAllStationsAvgHumidity();
    const url = `${this.apiUrl}?query=${encodeURIComponent(query)}`;
    return this.http.get<IWeather>(url);
  }
